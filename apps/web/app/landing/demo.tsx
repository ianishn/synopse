"use client";
/** Démo interactive et jouable (charte V1.0), bilingue via le dictionnaire COPY. */
...
export function Demo({ lang = "fr" }: { lang?: Lang }) {
    ...
    <div ref={rootRef} className="mx-auto max-w-md">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-line bg-surface shadow-[0_24px_70px_-24px_rgba(0,0,0,0.6)]">
        <div className="flex items-center gap-2 border-b border-line px-5 py-3">
          ...
          <span className="ml-auto font-mono text-[0.62rem] uppercase tracking-[0.16em] text-muted">{t.online}</span>
        </div>

        <div className="min-h-[300px] space-y-3 p-5 text-sm">
          {shown(["mail", "typing", "agent", "ask", "blocked", "allowed"]) && (
            <div className="pop-in">
              <p className="font-mono text-[0.62rem] uppercase tracking-[0.16em] text-muted">{t.inbox}</p>
              <div className="mt-1 rounded-2xl rounded-tl-sm bg-void-2 p-3">
                <p className="text-s400">{t.from}</p>
                <p className="mt-1 text-muted">{t.mailBody}</p>
                <p className="mt-2 rounded-lg bg-void p-2 font-mono text-[0.72rem] text-s400">{t.hidden}</p>
              </div>
            </div>
          )}
          ...
          {shown(["agent", "ask", "blocked", "allowed"]) && (
            <div className="pop-in rounded-2xl rounded-tl-sm border border-orange/30 bg-[var(--orange-soft)] p-3">
              ...
              <p className="mt-0.5 text-[0.72rem] text-muted">{t.files}</p>
            </div>
          )}

          {shown(["ask", "blocked", "allowed"]) && (
            <div className="pop-in rounded-2xl rounded-tl-sm border border-orange/40 bg-[var(--orange-soft)] p-3">
              <p className="font-medium text-off">{t.alert}</p>
              <p className="mt-0.5 text-[0.78rem] text-s400">{t.alertSub}</p>
              {!decided && (
          ...
          {phase === "blocked" && (
            <div className="pop-in rounded-2xl border border-orange/40 bg-void-2 p-3">
              <p className="flex items-center gap-2 font-medium text-off"><span className="h-2 w-2 rounded-full bg-orange" /> {t.blocked}</p>
              <p className="mt-1 text-[0.78rem] text-muted">{t.blockedSub}</p>
            </div>
          )}
          {phase === "allowed" && (
            <div className="pop-in rounded-2xl border border-line p-3">
              <p className="text-s400">{t.allowed}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="mt-5 text-center">
        {phase === "ask" && <p className="text-sm font-medium text-orange">{t.yourTurn}</p>}
        {decided && <button onClick={play} className="text-sm text-s400 underline underline-offset-4 hover:text-off">{t.replay}</button>}
        {!decided && phase !== "ask" && <p className="text-sm text-muted">{t.playing}</p>}
      </div>
    </div>
  );
}
