"use client";
/**
 * Fond WebGL animé "Warp stripes" (21st.dev Shader Builder). WebGL1, sans lib.
 * Triangle plein écran, DPR plafonné à 2, RAF en pause quand l'onglet est caché, curseur "repel".
 */
...
export function ShaderBackground({ className = "" }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const uColors = U("u_colors[0]"), uScene = U("u_scene"), uShape = U("u_shape"),
      uSurface = U("u_surface"), uFinish = U("u_finish"), uTransform = U("u_transform"),
      uSpace = U("u_space"), uCursor = U("u_cursor");

    const colors = new Float32Array(24);
    ([[0.086, 0.043, 0.043], [0.761, 0.251, 0.165], [0.957, 0.616, 0.216], [1.0, 0.91, 0.761]] as number[][])
      .forEach((c, i) => colors.set(c, i * 3));
    gl.uniform3fv(uColors, colors);
    gl.uniform4f(uShape, 2.36, 0.59, 0.56, 0.0);
    gl.uniform4f(uSurface, 0.8, 0.94, -0.27, 0.88);
    gl.uniform4f(uFinish, 0.0, 0.23, 0.012, 0.28);
    gl.uniform4f(uTransform, 7.0, 1.76, 0.16, 0.0);

    let w = 0, h = 0;
    const dpr = () => Math.min(2, window.devicePixelRatio || 1);
    const resize = () => {
      const r = dpr();
      w = Math.floor(canvas.clientWidth * r); h = Math.floor(canvas.clientHeight * r);
      canvas.width = w; canvas.height = h; gl.viewport(0, 0, w, h);
    };
    resize();
    window.addEventListener("resize", resize);

    // Curseur (repos = -1..1 en espace canvas). Présence lissée.
    let mx = 0, my = 0, pres = 0, targetPres = 0;
    const onMove = (e: PointerEvent) => {
      const rect = canvas.getBoundingClientRect();
      mx = ((e.clientX - rect.left) / rect.width) * 2 - 1;
      my = (1 - (e.clientY - rect.top) / rect.height) * 2 - 1;
      targetPres = 1;
    };
    const onLeave = () => { targetPres = 0; };
    ...
    const frame = () => {
      if (!running) return;
      const t = ((reduced ? 0 : performance.now() - start) / 1000) * 1.09;
      pres += (targetPres - pres) * 0.08;
      gl.uniform4f(uScene, w, h, t, 4.0);
      gl.uniform4f(uSpace, 0.0, 0.0, mx, my);
      gl.uniform4f(uCursor, pres, 1.0, 0.42, 0.47);
