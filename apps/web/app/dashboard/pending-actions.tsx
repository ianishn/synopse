...
export function PendingActions({ initial, lang }: { initial: Pending[]; lang: Lang }) {
  const ui = UI[lang];
  ...
  return (
    <section className="space-y-3">
      <h2 className="flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-amber-400">
        <span className="h-2 w-2 rounded-full bg-amber-500" /> {items.length} {items.length > 1 ? ui.pendingMany : ui.pendingOne}
      </h2>
      {items.map((it) => (
        <div key={it.id} className="rounded-xl border border-orange bg-void shadow-[0_0_0_1px_var(--orange),0_20px_50px_-30px_rgba(234,88,12,0.5)]">
          ...
          <div className="px-5 py-4">
            ...
            {it.rule && <p className="mt-2"><span className="inline-flex items-center gap-1.5 rounded-full border border-orange/30 bg-[var(--orange-soft)] px-2.5 py-0.5 text-xs font-medium text-orange-bright">{ui.rule} : {it.rule}</span></p>}
            {detail === it.id && (
              <p className="mt-3 rounded-lg border border-line bg-void-2 p-3 font-mono text-xs text-muted">
                Reçue le {new Date(it.created_at).toLocaleString(lang === "en" ? "en-GB" : "fr-FR")} · refus automatique à l&apos;expiration du compte à rebours (fail-safe).
              </p>
            )}
          </div>
          <div className="flex flex-wrap gap-2 px-5 pb-4">
            <button disabled={busy === it.id} onClick={() => decide(it.id, "denied")}
              className="rounded-full bg-orange px-4 py-2 text-sm font-semibold text-white transition hover:bg-orange-bright disabled:opacity-50">{ui.refuse}</button>
            <button disabled={busy === it.id} onClick={() => decide(it.id, "approved")}
              className="rounded-full border border-line px-4 py-2 text-sm font-medium text-off transition hover:border-s400 disabled:opacity-50">{ui.allowOnce}</button>
            <button onClick={() => setDetail(detail === it.id ? null : it.id)}
              className="rounded-full px-4 py-2 text-sm font-medium text-muted transition hover:text-off">{ui.seeDetail}</button>
          </div>
        </div>
      ))}
    </section>
