...
export function SpendPanel({ daily, monthTotal, monthlyCap, dailyCap, perAgent, plan, lang }:
  { daily: Daily[]; monthTotal: number; monthlyCap: number | null; dailyCap: number | null; perAgent: PerAgent[]; plan: string; lang: Lang }) {
  const ui = UI[lang];
  ...
  return (
    <section className="space-y-3">
      <h2 className="text-sm font-semibold uppercase tracking-wide text-muted">{ui.spend}</h2>
      <GlowCard>
        <div className="p-5">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="font-mono text-3xl tabular-nums text-off">{monthTotal.toFixed(2)} €</p>
              <p className="text-xs text-muted">{ui.spentThisMonth}{monthlyCap ? ` sur ${monthlyCap} €` : ""}</p>
            </div>
            {pct !== null && (
          ...
          {monthlyCap && (
            <ProgressBar className="mt-4" value={monthTotal} max={monthlyCap} danger={alert80}
              label={ui.capFeature} valueText={`${monthTotal.toFixed(2)} € / ${monthlyCap} €`} />
          )}

          {/* Graphique par JOUR (voir la dérive de la nuit) */}
          <div className="mt-5">
            <p className="mb-2 font-mono text-[0.62rem] uppercase tracking-widest text-muted">{ui.perDay}</p>
              ...
              <svg viewBox={`0 0 ${W} ${H}`} className="w-full" role="img" aria-label="Dépenses quotidiennes des 30 derniers jours">
                {daily.map((d, i) => {
                  ...
                  return <rect key={d.day} x={pad + i * bw + 1} y={H - h - 4} width={Math.max(1, bw - 2)} height={h}
                    ...
                    <title>{new Date(d.day).toLocaleDateString(lang === "en" ? "en-GB" : "fr-FR")} : {d.eur.toFixed(2)} €</title>
                  </rect>;
                })}
              </svg>
            ) : <p className="py-6 text-center text-sm text-muted">{ui.noSpend}</p>}
          </div>

          {perAgent.length > 0 && (
            <div className="mt-5 border-t border-line pt-4">
              <p className="mb-2 font-mono text-[0.62rem] uppercase tracking-widest text-muted">{ui.byAgent}</p>
            ...
            {paid ? (
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-dashed border-line px-4 py-3">
                <p className="text-sm text-muted">
                  <span className="font-medium text-s300">{ui.capFeature}</span>{ui.capDesc}
                </p>
